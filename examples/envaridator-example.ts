...
export const okURL = envaridator.register(
  'CORRECT_URL',
  toi.required().and(toix.str.url({ protocol: 'https:' })),
  'description for CORRECT_URL',
); // SHOULD PASS
export const wrongURL = envaridator.register(
  'WRONG_URL',
  toi.required().and(toix.str.url({ protocol: 'https:' })),
...
export const undefinedVariable = envaridator.register(
  'V@R1ABL3',
  toi.required().and(toi.str.is()),
  'description for V@R1ABL3',
); // SHOULD FAIL
export const existingVariable = envaridator.register(
  'SOME_VARIABLE',
  toi.required().and(toi.str.is()),
  'description for SOME_VARIABLE',
); // SHOULD PASS

process.env['SOME_OTHER_VARIABLE'] = 'I exist as well!';
export const objMyb = {
  VARIABLE3: envaridator.register(
    'SOME_OTHER_VARIABLE',
    toi.required().and(toi.str.is()),
    'description for SOME_THER_VARIABLE',
  ), // SHOULD PASS
};
