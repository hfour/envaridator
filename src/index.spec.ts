...
it('should not throw if all of the variables and rules are successfully validated', () => {
  ...
  envaridator.registerPostValidation('EXAMPLE_RULE', () => {}, 'Empty post validation rule');
...
it('should throw if a post validation rule fails', () => {
  ...
  expect(() => envaridator.validate()).toThrow(
    'The following environment variables are invalid:\n\nTHROW_RULE - This rule should throw',
  );
});
...
it('should describe all registered variables and rules', () => {
  ...
  envaridator.registerPostValidation('RULE_1', () => {}, 'The first rule. Must be configured correctly.');
...
RULE_1 - The first rule. Must be configured correctly."
`);
});
