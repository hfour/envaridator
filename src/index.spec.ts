...
it('should throw if a post validation rule fails', () => {
  const envaridator = new Envaridator();
  envaridator.registerPostValidation('This rule will throw when the envaridator validate is run', () => {
    throw new Error('This rule should throw');
  });

  expect(() => envaridator.validate()).toThrow('The following validation rules are invalid:\n\nThis rule should throw');
});
